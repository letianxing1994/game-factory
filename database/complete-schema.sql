-- ========================================
-- Game Factory Complete Database Schema
-- 包含所有表结构 + 双模型支持 + 默认数据
-- 版本: v1.3.0+
-- 最后更新: 2025-11-20
-- ========================================
...
-- 游戏开发团队表
CREATE TABLE IF NOT EXISTS game_development_teams (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
...
-- ========================================
-- 数据迁移：为现有agents添加双模型字段
-- （如果表已存在但缺少新字段）
-- ========================================

-- 检查并添加 ai_model_2d 字段
SET @col_exists = 0;
...
-- 检查并添加 ai_model_3d 字段
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists 
FROM INFORMATION_SCHEMA.COLUMNS 
WHERE TABLE_SCHEMA = DATABASE() 
  AND TABLE_NAME = 'agents' 
  AND COLUMN_NAME = 'ai_model_3d';

SET @sql = IF(@col_exists = 0,
    'ALTER TABLE agents ADD COLUMN ai_model_3d VARCHAR(50) COMMENT ''3D模型（用于生成3D资产）：Meshy-4, Luma AI等'' AFTER ai_model_2d',
    'SELECT ''ai_model_3d already exists'' AS status');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 对于2D美术，将ai_model复制到ai_model_2d
UPDATE agents 
SET ai_model_2d = ai_model 
WHERE type = 'artist' 
  AND dimension = '2d' 
  AND ai_model IS NOT NULL
  AND ai_model_2d IS NULL;

-- 对于3D美术，设置默认双模型
UPDATE agents 
SET ai_model_2d = COALESCE(ai_model, 'dall-e-3'),
    ai_model_3d = 'meshy-4'
WHERE type = 'artist' 
  AND dimension = '3d'
  AND ai_model_3d IS NULL;
...
-- 完成！
-- ========================================
SELECT '✅ 数据库架构已完整初始化！' AS status;
SELECT CONCAT('📊 共创建 ', COUNT(*), ' 张表') AS summary FROM information_schema.tables WHERE table_schema = DATABASE();
